...
#test use: python3 epmmm_get_mysql_stats-python3.py --servicename your_mysqlservice_name --serviceip 192.168.1.200 --serviceport 3306 --username zabbix --password zabbix
...
parser.add_argument("--servicename", action="store", dest='servicename', help="input the database servcie name", required=True)
parser.add_argument("--serviceip", action="store", dest='serviceip', help="input the database servcie ip", required=True)
...
parser.add_argument("--zabbix_type", action="store", dest='zabbix_type', help="zabbix_type", default='co')
...
logging.basicConfig(level=logging.INFO,
                    filename='/var/log/zabbix/epmmm-get-mysql-stats-%s.log' % (SERVICENAME),
                    datefmt='%Y/%m/%d %H:%M:%S',
...
def send_to_zabbix(packet, zabbix_host, zabbix_port):
    try:
        ...
        logger.error('epmmm: %s, %s %s, %s!' % (e, zabbix_host, zabbix_port, packet))

def generate_packet(SERVICENAME,resaultdic):
    packet = []
    for key in resaultdic.keys():
        packet.append(ZabbixMetric(SERVICENAME, "epmmm.mysql.%s" % key,str(resaultdic[key] if resaultdic[key]!='' else 0)))
...
def get_mysql_status(querysql):
    try:
        ...
        logger.error('epmmm %s, %s, %s!' % (SERVICENAME, e, querysql))
    else:
        return result
...
def to_float(rownum):
    ...
    try:
        ...
        logger.info('epmmm %s, %s, %s!' % (SERVICENAME, rownum, e))
        return 0.0
    return f

def to_int(rownum):
    rownum = re.findall('[-+?\d+\.\d]*',rownum)
    tmpint = int(float(rownum[0]))
    if tmpint >= 18446744073709551615:
        return 0
    else:
        return tmpint
...
def get_resaultdic():
    ...
    if resaults:
        MasterStatus['Slave_count'] = len(resaults)
    ...
    if resaults:
        for resault in resaults:
            ...
            if(resault['Slave_IO_Running']=='Yes' and resault['Slave_SQL_Running']=='Yes'):
                SlaveStatus['Slave_Running']=1
            else:
                SlaveStatus['Slave_Running']=0
    ...
    if resaults:
        for resault in resaults:
            if ( is_number(resault[1])):
                ...
                GlobalStatus[resault[0]]=resault[1]
        #mariadb
    ...
    if resaults:
        for resault in resaults:
            ...
            elif(resault[0]=='version_comment'):
                GlobalVariables['version_comment']=resault[1]
        ...
        if ('query_cache_type' in GlobalVariables):
        ...
        GlobalStatus['Innodb_buffer_pool_pages_misc_b'] = GlobalVariables['innodb_page_size'] * GlobalStatus['Innodb_buffer_pool_pages_misc']
    ...
    if (GlobalVariables['version_comment'] != 'MariaDB Server' and False):
    ...
    if 'group_replication_primary_member' in GlobalStatus:
        GlobalStatus['MGR_status'] = 1
        resaults=get_mysql_status("SELECT CASE MEMBER_ROLE WHEN 'PRIMARY' THEN 1 WHEN 'SECONDARY' THEN 0 ELSE 2 END 'MEMBER_ROLE' FROM performance_schema.replication_group_members where MEMBER_ID=@@server_uuid;")
        if resaults:
            for resault in resaults:
                GlobalStatus['MGR_member_role'] = resault[0]
    else:
        GlobalStatus['MGR_status'] = 0
    ...
    resaults=get_mysql_status_dic("SELECT `COUNT` FROM INFORMATION_SCHEMA.INNODB_METRICS WHERE NAME = 'trx_rseg_history_len'")
    if resaults:
        InnodbStatus['Innodb_trx_rseg_history_len'] = resaults[0]['COUNT']
...
def main():
    ...
    try:
        ...
        #print(packet)
        #logger.info(packet)
        #logger.info("%s send to %s %s" %(SERVICENAME, META_ZABBIX_SERVER_IP, META_ZABBIX_SERVER_PORT))
        send_to_zabbix(packet, META_ZABBIX_SERVER_IP, META_ZABBIX_SERVER_PORT)
    except Exception as e:
        logger.error('epmmm %s, %s!' % (SERVICENAME, e))
        logger.error(packet)
